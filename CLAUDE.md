...
- **Streak ≠ metas** (constância vs volume). Fuso fixo America/Sao_Paulo (`lib/streak.ts`). A chama 🔥 só acende se o mínimo de HOJE foi cumprido (ou se hoje não é dia que conta). **Dia bônus:** dia fora de `dias_que_contam` nunca quebra a sequência, mas se o mínimo foi cumprido nele, soma +1. **Início da semana das metas** é configurável (`user_settings.inicio_semana`: `'seg'` padrão | `'dom'`).
...
- Tags nos itens saem do campo **`items.fonte`** (`ads-pucpr` | `dio` | `alura` | `anthropic` | `coursera` | `deeplearning` | null), definido por GRUPO no seed (`SeedGroup.fonte`, item pode sobrescrever). Mapa de texto/cor em `components/ListaBlocos.tsx` (`TAGS_FONTE`): ADS-PUCPR bordô, DIO roxo, Alura azul #2A7AE2, Anthropic laranja, Coursera azul, DeepLearning.AI teal. Conceito sem fonte = sem tag (exceção: `review` sem fonte cai em ads-pucpr — fallback no `LinhaItem`). `optional` ganha a tag neutra "opcional". Nada de adivinhar por título de grupo. Chips de "onde estudar" usam OUTRO mapa (`PLATAFORMAS`): alura, puc, dio, anthropic, fiap (rosa #ED145B).
